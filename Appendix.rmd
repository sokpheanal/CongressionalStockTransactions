...
t1 %>%
  ...
  arrange(desc(count)) %>%
  kable(
    col.names = c('Gender', 'Count')) %>% #TO DO: Add caption
  kable_styling()

ggplot(t1, aes(x = gender, fill = gender)) +
  theme_bw() +
  geom_bar() + 
  scale_fill_brewer(palette = "Dark2") #TO DO: Add labs
```
...
t2 %>%
  ...
  arrange(desc(count)) %>%
  kable(
    col.names = c('Party', 'Count')) %>% #TO DO: Add caption
  kable_styling()
